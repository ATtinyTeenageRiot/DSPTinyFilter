...
def f2u(x):
    '''
    Quantization on 16 bits in signed fractionnal form
    min: 0x800000 == -1
    max: 0x7FFFFF is about 0.9999999988 or the like
    '''

    if np.abs(x - 1.0) < 1e-10:
        return int(2**15 - 1)
    elif x >= 0:
        return int(x*2.**15)
    else:
        return 2**16 + int(x*2**15)
...
for i in xrange(stages):
    ...
    print("sta321mp_biquad(codec, 1, %d, %f, %f, %f, %f, %f);" 
            % (i+2, b0/2, b1/2, b2, -a1/2, -a2))
    print("sta321mp_biquad(codec, 1, %d, 0x%x, 0x%x, 0x%x, 0x%x,0x%x);" 
            % (i+2, f2u(b0/2), f2u(b1/2), f2u(b2), f2u(-a1/2), f2u(-a2)))
